...
for t in `cat ${TMP}/dates.txt`; do
	...
	for j in `cat ${TMP}/names.txt`; do
		...
		r.import in=${SCN}_B10.TIF out=${SCN}_B10 memory=2000 resolution=region
		r.mapcalc "${SCN}_B10 = if(${SCN}_B10 == 0,null(),${SCN}_B10)" --o
		r.import in=${SCN}_B11.TIF out=${SCN}_B11 memory=2000 resolution=region
		r.mapcalc "${SCN}_B11 = if(${SCN}_B8 == 0,null(),${SCN}_B11)" --o
		if [ ${RES} -eq 30 ]; then
			...
			g.region n=${N} s=${S} w=${W} e=${E} res=${RES} -a
			i.fusion.hpf -l -c pan=${SCN}_B8 msx=${SCN}_B1,${SCN}_B2,${SCN}_B3,${SCN}_B4,${SCN}_B5,${SCN}_B6,${SCN}_B7,${SCN}_B9 center=high modulation=max trim=0.0 --o
			for i in 1 2 3 4 5 6 7 9; do
	...
	g.region n=${N} s=${S} w=${W} e=${E} res=${RES} zoom=${SEN}_p${PTH}_patch_${t}_T1_B5 -a
	i.colors.enhance red=${SEN}_p${PTH}_patch_${t}_T1_B5 green=${SEN}_p${PTH}_patch_${t}_T1_B4 blue=${SEN}_p${PTH}_patch_${t}_T1_B3 strength=95
	r.composite red=${SEN}_p${PTH}_patch_${t}_T1_B5 green=${SEN}_p${PTH}_patch_${t}_T1_B4 blue=${SEN}_p${PTH}_patch_${t}_T1_B3 output=${SEN}_p${PTH}_patch_${t}_T1_comp
