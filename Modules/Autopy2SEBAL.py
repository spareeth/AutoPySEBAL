...
@click.argument('itype')
@click.argument('pathdem')
@click.argument('lsprefix')
@click.argument('lsnr')
@click.argument('lsthermal')
@click.argument('hot')
@click.argument('cold')
#meteo
...
#soil
@click.argument('theta_sat_top')
@click.argument('theta_sat_sub')
@click.argument('theta_res_top')
@click.argument('theta_res_sub')
@click.argument('wilt_pt')
@click.argument('depl_factor')
...
def main(indir, outdir, itype, pathdem, lsprefix, lsnr, lsthermal, hot, cold, tinst, t24, rhinst, rh24, winst, w24, zx, rad_method24, rs24, transm24, rad_method_inst, rsinst, transminst, obst_ht, theta_sat_top, theta_sat_sub, theta_res_top, theta_res_sub, wilt_pt, depl_factor, field_capacity, luemax, tcoldminin1, tcoldmaxin1, ndvihot_low1, ndvihot_high1, temp_lapse, res):
    ...
    Image_Type = int(itype)           # Type of Image (1=Landsat & 2 = VIIRS & GLOBA-V)     
    ...
    if Image_Type == 1:	
        ...
        Name_Landsat_Image = str(lsprefix)    # From glovis.usgs.gov
        Landsat_nr = int(lsnr)            # Type of Landsat (LS) image used (LS5, LS7, or LS8)
        Bands_thermal = int(lsthermal)         # Number of LS bands to use to retrieve land surface 
                                    
                                                             # temperature: 1 = Band 6 for LS_5 & 7, Band 10 for LS_8 (optional)
        # Calibartion constants Hot Pixels from the excel file 
        Hot_Pixel_Constant = float(hot)          # Hot Pixel Value = Mean_Hot_Pixel + Hot_Pixel_Constant * Std_Hot_Pixel (only for Landsat images)
  
        # Calibartion constants Cold Pixels from the excel file 
        Cold_Pixel_Constant = float(cold)         # Cold Pixel Value = Mean_Cold_Pixel + Cold_Pixel_Constant * Std_Cold_Pixel (only for Landsat images)
    ...
    try:
        h_obst = float(obst_ht)                # Obstacle height
    ...
    try:
        Theta_sat_top = float(theta_sat_top)                # Saturated soil moisture content topsoil
    ...
    # if the data is not a value, than open as a string
        ...
        Theta_sat_top_name = '%s' %str(theta_sat_top) 
    ...
    # Open soil data, first try to open as value, otherwise as string (path)								
    try:
        Theta_sat_sub = float(theta_sat_sub)                # Saturated soil moisture content subsoil
    ...
    # if the data is not a value, than open as a string
        ...
        Theta_sat_sub_name = '%s' %str(theta_sat_sub) 
    ...
    # Open soil data, first try to open as value, otherwise as string (path)															
    try:
        Theta_res_top = float(theta_res_top)              # Residual soil moisture content
    ...
    # if the data is not a value, than open as a string													
        ...
        Theta_res_top_name = '%s' %str(theta_res_top) 
    ...
    # if the data is not a value, than open as a string								
        ...
        Theta_res_sub_name = '%s' %str(theta_res_sub) 
    ...
    # Open soil data, first try to open as value, otherwise as string (path)											
    try:
        Soil_moisture_wilting_point = float(wilt_pt)              # Wilting point
    ...
    # if the data is not a value, than open as a string								
        ...
        Soil_moisture_wilting_point_name = '%s' %str(wilt_pt) 
    ...
    # Open soil data, first try to open as value, otherwise as string (path)											
    try:
        depl_factor = float(depl_factor)              # Depletion factor
    ...
    # if the data is not a value, than open as a string								
        ...
        depl_factor_name = '%s' %str(depl_factor) 
    ...
    # Open soil data, first try to open as value, otherwise as string (path)		   
    try:
        Field_Capacity = float(field_capacity)              # Field capacity divided by saturation capacity
    ...
    # if the data is not a value, than open as a string
        ...
        Field_Capacity_name = '%s' %str(field_capacity) 
    ...
    # Open soil data, first try to open as value, otherwise as string (path)		   
    try:
        LUEmax = float(luemax)              # Field capacity divided by saturation capacity
